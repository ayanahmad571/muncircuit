...
####
		...
		if($loga==1){
			if($adm ==1){
				$sql = "SELECT * FROM `mun_modules` WHERE `mo_ifadmin`=1 and `mo_if_log_in`=1 and mo_valid =1";

			}else{
				$sql = "SELECT * FROM `mun_modules` WHERE `mo_ifnoadmin`=1 and `mo_if_log_in`=1 and mo_valid =1";

			}
		}else if($loga==0){
			
$sql = "SELECT * FROM `mun_modules` WHERE `mo_ifnoadmin`=1 and `mo_if_no_log_in`=1 and mo_valid =1";
		}
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
		...
		$trem = getdatafromsql($conn,"select count(rpt_id) as reps from mun_reports where rpt_resolved = 0 and rpt_valid =1 ");
		if(is_array($trem)){
			$reportsfound = $trem['reps'];
		}else{
			$reportsfound = 0;
		}
	}
	...
	$trrem = getdatafromsql($conn,"select count(ml_id) as dddda from mun_mails where ml_read = 0 and ml_valid =1 ");
		...
		if($row['mo_sub_mod'] == 1){
			...
			$actcswl = "SELECT * FROM `mun_sub_mods` WHERE `sub_mo_rel_mo_id`= ".$row['mo_id']." and sub_valid=1 and sub_href='".trim(basename($_SERVER['PHP_SELF']))."'";
...
if ($act_res->num_rows == 1) {
	...
	echo '<ul class="list-unstyled">
			...
			$submod = "SELECT * FROM `mun_sub_mods` where sub_valid = 1 and sub_mo_rel_mo_id='".$row['mo_id']."'";
...
function make_user_ar($conn,$lum,$lo){
	...
	$sql = "select * from mun_logins l left join mun_users u on
	l.lum_id = u.usr_rel_lum_id where l.lum_id = ".$lum." and l.lum_valid =1";
...
function preplogs($array,$userid, $tablename ,$querytype, $newfullquery,$conn,$shh){
	$egijurtegii = '';
		...
		if($conn->query("INSERT INTO `mun_user_logs`( `ul_session_hash`, `ul_rel_usr_id`, `ul_ins_dnt`, `ul_table`, `ul_content`, `ul_querytype`) VALUES (
		'".$shh."',
		'".$userid."',
		'".time()."',
		'".$tablename."',
